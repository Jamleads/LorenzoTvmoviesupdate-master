...
const AnimationDetails = ({
  ...
  const movie = animationFromDb?.filter((movie) => movie.title === title)[0];

  const { animationImagesFromDb } = useAppContext();

  const filteredUrls = animationImagesFromDb?.filter((item) => {
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-b from-[#020d18] to-[#111827] md:bg-[#020d18] text-slate-100 px-4 md:px-[50px] pt-[130px] pb-4">
        ...
        <div className="w-full bg-black/30 mt-10 mb-8 p-5 block sm:flex gap-10 rounded-lg">
          <img
            alt=""
